...
integration:
	go test $(TEST_SOURCES_NO_CUCUMBER)
	cd test && go test --godog.strict=true --godog.format=pretty --godog.tags="@algod,@assets,@auction,@kmd,@send,@template,@indexer,@rekey,@dryrun,@compile,@applications.verified" --test.v .
