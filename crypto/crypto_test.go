...
func TestMakeLogicSigBasic(t *testing.T) {
	...
	require.Equal(t, ed25519.PublicKey(nil), lsig.SigKey)
...
func TestMakeLogicSigSingle(t *testing.T) {
	...
	require.Equal(t, acc.PublicKey, lsig.SigKey)
	require.True(t, lsig.Msig.Blank())
...
func TestSignLogicsigTransaction(t *testing.T) {
	...
	testSign := func(t *testing.T, lsig types.LogicSig, sender types.Address, expectedBytes []byte, expectedTxid string, expectedAuthAddr types.Address) {
		...
		require.Equal(t, lsig.Logic, stx.Lsig.Logic)
		if len(lsig.Args) != 0 {
			require.Equal(t, lsig.Args, stx.Lsig.Args)
		} else {
			require.Equal(t, 0, len(stx.Lsig.Args))
		}
		require.Equal(t, lsig.Msig, stx.Lsig.Msig)
		require.Equal(t, lsig.Sig, stx.Lsig.Sig)
		require.Equal(t, ed25519.PublicKey(nil), stx.Lsig.SigKey)
