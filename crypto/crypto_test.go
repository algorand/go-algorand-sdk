...
func makeTestMultisigAccount(t *testing.T) (MultisigAccount, Ed25519Signer, Ed25519Signer, Ed25519Signer) {
	...
	require.NoError(t, err)
	sgnr1, err := SKToInMemorySigner(sk1)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	sgnr2, err := SKToInMemorySigner(sk2)
	require.NoError(t, err)
	...
	sgnr3, err := SKToInMemorySigner(sk3)
	require.NoError(t, err)
...
func TestSignMultisigTransaction(t *testing.T) {
	...
	txid, txBytes, err := Ed25519SignMultisigTransaction(sgnr1, ma, tx)
	require.NoError(t, err)
...
func TestAppendMultisigTransaction(t *testing.T) {
	...
	ma, _, sgnr2, _ := makeTestMultisigAccount(t)
	txid, txBytes, err := Ed25519AppendMultisigTransaction(sgnr2, ma, uniSigTxBytes)
	require.NoError(t, err)
...
func TestSignMultisigTransactionKeyReg(t *testing.T) {
	...
	ma, sgnr1, _, _ := makeTestMultisigAccount(t)
	txid, txBytes, err := Ed25519SignMultisigTransaction(sgnr1, ma, tx.Txn)
	require.NoError(t, err)
...
func TestAppendMultisigTransactionKeyReg(t *testing.T) {
	...
	ma, _, _, sgnr3 := makeTestMultisigAccount(t)
	txid, txBytes, err := Ed25519AppendMultisigTransaction(sgnr3, ma, uniSigTxBytes)
	require.NoError(t, err)
...
func TestSignBytes(t *testing.T) {
	...
	sgnr, err := SKToInMemorySigner(account.PrivateKey)
	require.NoError(t, err)
	message := make([]byte, 15)
	rand.Read(message)
	signature, err := Ed25519SignBytes(sgnr, message)
	require.NoError(t, err)
...
func TestMakeLogicSigSingle(t *testing.T) {
	...
	sgnr, err := SKToInMemorySigner(ed25519.PrivateKey{0xd2, 0xdc, 0x4c, 0xcc, 0xe9, 0x98, 0x62, 0xff, 0xcf, 0x8c, 0xeb, 0x93, 0x6, 0xc4, 0x8d, 0xa6, 0x80, 0x50, 0x82, 0xa, 0xbb, 0x29, 0x95, 0x7a, 0xac, 0x82, 0x68, 0x9a, 0x8c, 0x49, 0x5a, 0x38, 0x5e, 0x67, 0x4f, 0x1c, 0xa, 0xee, 0xec, 0x37, 0x71, 0x89, 0x8f, 0x61, 0xc7, 0x6f, 0xf5, 0xd2, 0x4a, 0x19, 0x79, 0x3e, 0x2c, 0x91, 0xfa, 0x8, 0x51, 0x62, 0x63, 0xe3, 0x85, 0x73, 0xea, 0x42})
	require.NoError(t, err)
	program = []byte{1, 32, 1, 1, 34}
	lsig, err := makeLogicSig(program, args, sgnr, pk)
	require.NoError(t, err)
	...
	verified := VerifyLogicSig(lsig, types.Address(sgnr.Ed25519PublicKey()))
	require.True(t, verified)

	// check that a modified program fails verification
	modProgram := make([]byte, len(program))
	copy(modProgram, program)
	lsigModified, err := makeLogicSig(modProgram, args, sgnr, pk)
	require.NoError(t, err)
	modProgram[3] = 2
	verified = VerifyLogicSig(lsigModified, types.Address(sgnr.Ed25519PublicKey()))
	require.False(t, verified)
...
func TestMakeLogicSigMulti(t *testing.T) {
	...
	acc := GenerateAccount()
	signer, err := SKToInMemorySigner(acc.PrivateKey)
	require.NoError(t, err)
	...
	err = Ed25519AppendMultisigToLogicSig(&lsig, signer)
	require.Error(t, err) // sk not part of multisig

	err = Ed25519AppendMultisigToLogicSig(&lsig, sgnr2)
	require.NoError(t, err)
	...
	modProgram := make([]byte, len(program))
	copy(modProgram, program)
	lsigModified, err := makeLogicSig(modProgram, args, sgnr1, ma)
	require.NoError(t, err)
	modProgram[3] = 2
	...
	lsigf, err := makeLogicSig(program, args, signer, pk)
	require.NoError(t, err)
	lsig.Sig = lsigf.Sig
...
func TestSignLogicSigAccountTransaction(t *testing.T) {
	...
	t.Run("single sig", func(t *testing.T) {
		...
		lsigAccount, err := Ed25519MakeLogicSigAccountDelegated(program, args, sgnr)
		require.NoError(t, err)
		...
		t.Run("sender is contract addr", func(t *testing.T) {
			...
			expectedTxid := "XPFTYDOV5RCL7K5EGTC32PSTRKFO3ITZIL64CRTYIJEPHXOTCCXA"
			expectedAuthAddr := types.Address{}
			sender := types.Address(sgnr.Ed25519PublicKey())
			testSign(t, lsigAccount, sender, expectedBytes, expectedTxid, expectedAuthAddr)
		})
		...
		t.Run("sender is not contract addr", func(t *testing.T) {
			...
			expectedTxid := "U4X24Q45MCZ6JSL343QNR3RC6AJO2NUDQXFCTNONIW3SU2AYQJOA"
			expectedAuthAddr := types.Address(sgnr.Ed25519PublicKey())
			sender := otherAddr
			testSign(t, lsigAccount, sender, expectedBytes, expectedTxid, expectedAuthAddr)
		})
	})
...
func TestTealSign(t *testing.T) {
	...
	sig1, err := Ed25519TealSign(sgnr, data, addr)
	require.NoError(t, err)

	sig2, err := Ed25519TealSignFromProgram(sgnr, data, prog)
	require.NoError(t, err)
