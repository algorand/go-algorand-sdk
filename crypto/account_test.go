...
func TestLogicSigAddress(t *testing.T) {
	...
	t.Run("single sig", func(t *testing.T) {
		...
		signer, err := SKToInMemorySigner(account.PrivateKey)
		require.NoError(t, err)
	...
	t.Run("multi sig", func(t *testing.T) {
		...
		lsig, err := makeLogicSig(program, args, sgnr1, ma)
		require.NoError(t, err)
...
func TestMakeLogicSigAccount(t *testing.T) {
	...
	t.Run("Delegated", func(t *testing.T) {
		signer, err := SKToInMemorySigner(ed25519.PrivateKey{0xd2, 0xdc, 0x4c, 0xcc, 0xe9, 0x98, 0x62, 0xff, 0xcf, 0x8c, 0xeb, 0x93, 0x6, 0xc4, 0x8d, 0xa6, 0x80, 0x50, 0x82, 0xa, 0xbb, 0x29, 0x95, 0x7a, 0xac, 0x82, 0x68, 0x9a, 0x8c, 0x49, 0x5a, 0x38, 0x5e, 0x67, 0x4f, 0x1c, 0xa, 0xee, 0xec, 0x37, 0x71, 0x89, 0x8f, 0x61, 0xc7, 0x6f, 0xf5, 0xd2, 0x4a, 0x19, 0x79, 0x3e, 0x2c, 0x91, 0xfa, 0x8, 0x51, 0x62, 0x63, 0xe3, 0x85, 0x73, 0xea, 0x42})
		require.NoError(t, err)

		lsigAccount, err := Ed25519MakeLogicSigAccountDelegated(program, args, signer)
		require.NoError(t, err)
		...
		require.Equal(t, ed25519.PublicKey(pk[:]), lsigAccount.SigningKey)

		require.True(t, lsigAccount.IsDelegated())
	})

	t.Run("DelegatedMsig", func(t *testing.T) {
		ma, sgnr1, sgnr2, _ := makeTestMultisigAccount(t)

		lsigAccount, err := Ed25519MakeLogicSigAccountDelegatedMsig(program, args, ma, sgnr1)
		require.NoError(t, err)

		addr, err := ma.Address()
		require.NoError(t, err)

		expectedSigBytes, err := sgnr1.Ed25519Sign(bytes.Join(
		...
		t.Run("AppendMultisigSignature", func(t *testing.T) {
			err := lsigAccount.Ed25519AppendMultisigSignature(sgnr2)
			require.NoError(t, err)

			expectedSigBytes, err := sgnr2.Ed25519Sign(bytes.Join(
				[][]byte{[]byte("MsigProgram"), addr[:], program}, nil,
			))
			require.NoError(t, err)
			var expectedSig types.Signature
...
func TestLogicSigAccountFromLogicSig(t *testing.T) {
	...
	t.Run("single sig", func(t *testing.T) {
		...
		t.Run("with correct public key", func(t *testing.T) {
			signerpk := signer.Ed25519PublicKey()
			pk := ed25519.PublicKey(signerpk[:])
			lsigAccount, err := LogicSigAccountFromLogicSig(lsig, &pk)
			require.NoError(t, err)

			require.Equal(t, lsig, lsigAccount.Lsig)
			require.Equal(t, pk, lsigAccount.SigningKey)
		...
		t.Run("with incorrect public key", func(t *testing.T) {
			pk := signer.Ed25519PublicKey()
			var wrongPublicKey = make(ed25519.PublicKey, len(pk))
			copy(wrongPublicKey, pk[:])
...
func TestLogicSigAccount_Address(t *testing.T) {
	...
	t.Run("single sig", func(t *testing.T) {
		...
		actualAddr, err := lsigAccount.Address()
		require.NoError(t, err)
		require.Equal(t, types.Address(signer.Ed25519PublicKey()), actualAddr)
	})
